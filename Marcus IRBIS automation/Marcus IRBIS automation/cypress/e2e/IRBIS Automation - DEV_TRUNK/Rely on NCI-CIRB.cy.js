describe("Create Rely on NCI-CIRB Application", () => {
  ...
  it("Create Rely on Commerical IRB", () => {
    ...
    cy.get('[qid="q_2230"] > .copy > #q_74_2230').type(
      "My Rely On NCI-CIRB A.4.A.1 test"
    );
    ...
    cy.get('[qid="q_2232"] > .copy > #q_74_2232').type(
      "My Rely On NCI-CIRB IND/IDE test"
    );
    cy.get('[qid="q_2233"] > td.copy > #q_74_2233').type(
      "My Rely On NCI-CIRB Brief Description test"
    );
    cy.get(
    ...
    cy.get(
      '.even > td.copy > [style="position: relative; display: block;"] > div > .copy'
    ).click();
    cy.get(
    ...
    cy.get("#q_79_957").type(
      "My Rely On NCI-CIRB B.2.1 test - List the data you are planning to collect for this purpose."
    );
    cy.get("#tbl_79_956 > tbody > :nth-child(2) > td.copy").should(
      "be.visible"
    );
    cy.get("#q_79_958").type(
      "My Rely On NCI-CIRB B.2.1 test - Describe how confidentiality/privacy will be protected prior to ascertaining the patients willingness to participate"
    );
    cy.get("#tbl_79_956 > tbody > :nth-child(3) > td.copy").should(
      "be.visible"
    );
    cy.get("#q_79_959").type(
      "My Rely On NCI-CIRB B.2.1 test - Describe when and how you will destroy PHI for individuals"
    );
    cy.get(":nth-child(4) > td.copy").should("be.visible");
    cy.get("#q_79_2293").type(
      "My Rely On NCI-CIRB B.2.1 test - Please explain how screening could not be practicably conducted without the waiver."
    );
    cy.get(":nth-child(5) > td.copy").should("be.visible");
    cy.get("#q_79_2294").type(
      "My Rely On NCI-CIRB B.2.1 test - Explain why it would not be possible to conduct the screening without the use of PHI"
    );
  ...
  it("Rely on NCI-CIRB IRB Being Routed/PI Cert", () => {
    ...
    cy.get('input[placeholder="Title"]').type("My Rely on NCI-CIRB IRB test");
  ...
  it("Rely on NCI-CIRB IRB - Local/PRC Dept Approval", () => {
    ...
    cy.get('input[placeholder="Title"]').type(
      "My Rely on NCI-CIRB IRB Cypress Test"
    );
    cy.get(".sorting_1 > a").click();
    cy.get('[style="padding:10px;"] > div.searchresults').should(
      "contain",
  ...
  it("Verify Study History Screen", () => {
    ...
    cy.get('[rowspan="2"]').should(
      "contain",
      "My Rely on NCI-CIRB Cypress Test"
    );
