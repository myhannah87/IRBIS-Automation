describe("", () => {
  beforeEach(() => {
    ...
    cy.get("input#password.long").type("Lordoflords!{Enter}");
  });
  ...
  it("NEW STUDY LINK / CARDS", () => {
    ...
    cy.get(".bd").should("be.visible");
    cy.get(".sorting_desc > .DataTables_sort_wrapper > label").should(
      "be.visible"
    );
    cy.get(
      '[aria-label="Title: activate to sort column ascending"] > .DataTables_sort_wrapper > label'
    ).should("be.visible");
    cy.get(
      '[aria-label="PI: activate to sort column ascending"] > .DataTables_sort_wrapper > label'
    ).should("be.visible");
    cy.get(
      '[aria-label="Most RecentSubmission: activate to sort column ascending"] > .DataTables_sort_wrapper > label'
    ).should("be.visible");
    cy.get(
      '[aria-label="IRB: activate to sort column ascending"] > .DataTables_sort_wrapper > label'
    ).should("be.visible");
    cy.get(
      '[aria-label="Study Status: activate to sort column ascending"] > .DataTables_sort_wrapper > label'
    ).should("be.visible");
    cy.get(
      '[aria-label="ExpirationDate: activate to sort column ascending"] > .DataTables_sort_wrapper > label'
    ).should("be.visible");
    cy.get(
      '[aria-label="Admin AnnualReview Date: activate to sort column ascending"] > .DataTables_sort_wrapper > label'
    ).should("be.visible");
    cy.get(
      '[aria-label="Copy: activate to sort column ascending"] > .DataTables_sort_wrapper'
    ).should("be.visible");
    //RENEWAL
