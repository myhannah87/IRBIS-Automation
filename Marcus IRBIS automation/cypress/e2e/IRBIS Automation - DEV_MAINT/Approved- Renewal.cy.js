describe("Approved - Renewal", () => {
  const submissionOwner = "yulissa";
  const PI = "shilohma";
  const IRBAnalyst = "jfuse";
  const IRBNumber = "24-0742";

  it.only("Create Renewal", () => {
    ...
    cy.get(".hdWizard").should("contain", "Create a Renewal");

    cy.get("#wizard7 > .wizardType").should(
      "contain",
      "No Changes"
    );
    cy.get("#wizard8 > .wizardType").should(
      "contain",
      "Personnel Modification Only"
    );
    cy.get("#wizard9 > .wizardType").should(
      "contain",
      "Study  Modification"
    );
