describe("Login as Committee Member", () => {
  ...
  const committeeReviewScreen = 'https://orisstage.research.unc.edu/irb/index.cfm?event=committee.reviewers&committeeId=12&reviewDate=01/10/2023'
  const fullBoardReview = 'https://orisstage.research.unc.edu/irb/index.cfm?event=admin.reviewWindow&reviewId=530098&appId=413332&committeeView=1'
  ...
  it("Committee Reviewer Dashboard", () => {
    ...
    cy.get(".headerDescription").should(
      "contain",
      "Select a committee on the left to view the Agenda, Minutes and submissions for the committee selected."
    );
    cy.get(
      "#committeeReviews > :nth-child(1) > :nth-child(1) > :nth-child(1) > :nth-child(1)"
    ).should("be.visible");
    cy.get(":nth-child(1) > .sideNavHeader").should(
      "contain",
      "Next Committee"
    );
    cy.get('.oddEvenContainer > [data-committeeid="2"]').should(
      "contain",
      "Board A"
    );
    ...
    cy.get(":nth-child(2) > .sideNavHeader").should(
      "contain",
      "Search Committees"
    );
    ...
    //View Current Agenda:
    cy.get(
      '[href="index.cfm?event=admin.committee.exportPDF&type=agenda&idValue=0&committeeId=12&reviewDate=01/10/2023"]'
    ).should("contain", "View Current Agenda");
    //View Current Minutes:
    cy.get(
      '[href="index.cfm?event=admin.committee.exportPDF&type=minutes&idValue=0&committeeId=12&reviewDate=01/10/2023"]'
    ).should("contain", "View Current Minutes");
    ...
    cy.get('[aria-label="Title: activate to sort column ascending"]').should(
      "contain",
      "Title"
    );
    cy.get('[aria-label="PI: activate to sort column ascending"]').should(
      "contain",
      "PI"
    );
    cy.get(
      '[aria-label="Submission Type: activate to sort column ascending"]'
    ).should("contain", "Submission Type");
    cy.get(
      '[aria-label="Date Received: activate to sort column ascending"]'
    ).should("contain", "Date Received");
    cy.get(
      '[aria-label="Review Type: activate to sort column ascending"]'
    ).should("contain", "Review Type");
    cy.get(
      '[aria-label="Primary Reviewer: activate to sort column ascending"]'
    ).should("contain", "Primary Reviewer");
    cy.get(
      '[aria-label="Secondary Reviewer: activate to sort column ascending"]'
  ...
  it("Committee Reviewer Application pop up", () => {
    cy.viewport(1400, 750);
    //cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=committee.reviewers&committeeId=12&reviewDate=01/10/2023');
    cy.visit(committeeReviewScreen)
    cy.get("input#username.long")
      .type(committeeMember)
      .should("have.value", committeeMember);
    ...
    */
      ...
      //launch Rrimary Reviewer popup
       ...
       cy.get("#frmUploadOHREAttachment").and(
         "contain",
         "Click Browse to select a Reviewer Checklist:"
       );
    ...
    cy.get(".tableheadersub3 > :nth-child(1)").should(
      "contain",
      "Review Result"
    );
    ...
    cy.get(".tableheadersub3 > :nth-child(4)").should(
      "contain",
      "Document List"
    );
    cy.get(".tableheadersub3 > :nth-child(5)").should(
      "contain",
      "PI Responses"
    );
    cy.get("tbody > :nth-child(2) > :nth-child(5) > a").should(
      "contain",
      "View"
    );
    cy.get(".button").click();
    ...
    cy.get(
      "#questionPIResponse1856 > :nth-child(2) > .stipDiv > :nth-child(2)"
    ).should("be.visible");
    cy.get(
      "#questionPIResponse1856 > :nth-child(2) > .responseDiv > :nth-child(1)"
    ).should("be.visible");
    cy.get(
      "#questionPIResponse1856 > :nth-child(2) > .responseDiv > :nth-child(2)"
    ).should("be.visible");
    cy.get(
      "#questionPIResponse1856 > :nth-child(2) > .actionDiv > :nth-child(1)"
    ).should("be.visible");
    cy.get(
      "#questionPIResponse1856 > :nth-child(2) > .actionDiv > :nth-child(2)"
    ).should("be.visible");
   ...
   //VERIFY REVIEW RESULTS
    ...
    cy.get("#approvalDateContainer > .reviewResultContainer > div").should(
      "contain",
      "01/10/2023"
    );
    cy.get("#expirationDateContainer > span").should(
      "contain",
      "Expiration Date"
    );
    cy.get("#expirationDateContainer > .reviewResultContainer > div").should(
      "contain",
      "01/09/2024"
    );
    */
    ...
    cy.get(":nth-child(2) > .subHeader").should(
      "contain",
      "Study Specific Findings"
    );
    cy.get(":nth-child(3) > .subHeader").should(
      "contain",
      "Submission Specific Findings"
    );
    cy.get(":nth-child(5) > .subHeader").should(
      "contain",
