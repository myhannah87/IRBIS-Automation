describe("Exempt - Modification", () => {
  it("Create Modification", () => {
    ...
    //cy.visit('https://orisdev.research.unc.edu/irb/eform_screens.cfm?MasterId=383839&ScreenId=99');
    ...
    cy.get('[qid="h_555"] > [width="99%"]').should(
      "contain",
      "Provide a brief non-technical summary of any changes you will be making to the study"
    );
    ...
    cy.get("#q_99_2319").should("be.visible").type("My justification for 3a.");
    //question 3b - yes
    ...
    cy.get('#hd_2164\\|\\|Yes > [width="99%"]').should(
      "contain",
    ...
    cy.get('#hd_2164\\|\\|No > [width="99%"]').should(
      "contain",
      "Please provide justification below:"
    );
    ...
    cy.get(
      ...
      "contain",
      "Otolaryngology Simulation Lab for Preclinical Medical Students"
    );
    cy.get(
      "#modalPopup > .tablemainsub-allborders > tbody > :nth-child(2) > .copy"
    ).should("contain", "Christie DeMason");
  ...
  it("Exempt Mod - Being Routed", () => {
    cy.visit(
      "https://orisdev.research.unc.edu/irb/index.cfm?event=general.logout"
    );
    cy.get("input#username.long")

      //UPDATE LOG IN TO REFERENCE PI ON IRB 22-2442
      .type("cdemason")
      .should("have.value", "cdemason");
    cy.get("input#password.long").type("test{Enter}");
...
it("Waiting PI Repsonse", () => {
  ...
  cy.get("input#username.long").type("khwhite").should("have.value", "khwhite");
  cy.get("input#password.long").type("test{Enter}");
