describe("REPORTING", () => {
  it("Verify Reporting landing screen", () => {
    ...
    cy.get(":nth-child(9) > dd.even > a").should(
      "contain",
      "Submission Report"
    );
    ...
    cy.get('[style="float: left; width: 48%;"] > h3').should(
      "contain",
      "FINALIZED SUBMISSIONS BY TYPE Past 7 Days"
    );

    cy.get('[style="float: right; width: 48%;"] > h3').should(
      "contain",
    ...
    cy.get('[style="float: left;"] > h3').should(
      "contain",
      "ACTIVE SUBMISSION COUNT BY ANALYST"
    );
    cy.get(
      '[href="index.cfm?event=irbis_reporting.irb_reports.index&dashboard=all"]'
    ).should("contain", "All");
    cy.get(
      '[href="index.cfm?event=irbis_reporting.irb_reports.index&dashboard=ohre"]'
    ).should("contain", "Under Review");
    cy.get(
      '[href="index.cfm?event=irbis_reporting.irb_reports.index&dashboard=pi"]'
    ).should("contain", "Waiting PI Response");
  });
  ...
  it("Study Report", () => {
    ...
    cy.viewport(1400, 1100);
    cy.get("#dashboardmenu2 > dd > a")
      .should("contain", "Study Report")
      .click();
    cy.get(
    ...
    cy.get(
      '#editSearchCriteria99 > .searchCriteriaForm > .booleanDiv > [value="1"]'
    ).click();
    cy.get(
      "#editSearchCriteria99 > .searchCriteriaForm > .editCritButtons > .btnSubmitCrit"
    ).click();
    ...
    cy.get(
      "#editSearchCriteria99 > .searchCriteriaForm > .hideOperator > .selectOperatorDiv > .addSearchOperator"
    )
    ...
    cy.get(
      "#editSearchCriteria99 > .searchCriteriaForm > .selectValuesDiv > .listValues > .lookupList"
    ).click();
