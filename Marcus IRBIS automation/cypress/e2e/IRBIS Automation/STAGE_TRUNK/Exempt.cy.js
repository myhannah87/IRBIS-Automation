...
//CONSENT PROCESS FOR EXEMPTIONS
  ...
  //1. GENERAL INFORMATION SCREEN
    ...
    cy.get('#q_29_3').should("be.visible")
      .type('My Exempt Cypress Test')//.should('have.value', 'My Exempt Regression Test') 
...
it('Accept Exempt For Review', () => {  
   ...
   cy.wait(1000)
   cy.get('#bucketDataTable_filter > label > input').type('My Exempt Cypress Test')
...
it.only('Revised-Resubmitted to IRB', () => {
   ...
   // CAPTURE URL OF REVIEW WINDOW
    ...
    cy.get('#reviewResultCategory12').should('contain', '1.Educational setting')
    cy.get('#reviewResultCategory13').should('contain', '2.Survey, interview, public observation')
    cy.get('#reviewResultCategory14').should('contain', '3. Benign Behavioral intervention')
    cy.get('#reviewResultCategory15').should('contain', '4. Secondary data/specimens')
    cy.get('#reviewResultCategory16').should('contain', '5.Federal demonstration projects')
    cy.get('#reviewResultCategory17').should('contain', '6.Taste or food preference')
