describe('Protocol Continuation',  () => {
   it('Create Protocol Continuation', () => {
    ...
    cy.get('input#username.long')
      .type('savlynch').should('have.value', 'savlynch')  
    ...
    cy.get(':nth-child(4) > .copy').should('contain', 'A Comparison of Two Brief Suicide Prevention Interventions Tailored for Youth on the Autism Spectrum')

    //PROTOCOL CONTINUATION
    cy.get('#formBody > .pageHeaderTitle').should('contain', 'Protocol Continuation')
    cy.get('.introText > :nth-child(1)').should('contain', 'The IRB can grant a continuation of some or all research activities with currently enrolled subjects during lapse of IRB approval for a therapeutic or treatment study where the IRB has determined there is a prospect for direct benefit.')
    cy.get('.introText > :nth-child(2)').should('contain', 'List below for each participant their study identification code (Study ID) and the specific study activities that you propose to continue as well as the clinical/medical justification for the continuation and the risks to the participant if study activities are not continued.')
    cy.get('em').should('contain', 'Do not provide any direct participant identifiers. Activities can be general, e.g., administration of study drug, safety labs, imaging, etc.')
    cy.get('#noParticipants > p').should('contain', 'There are currently no participants.')
    
      //Add Participant
    cy.get('#btnSave').click()
    
    cy.get('.ui-dialog').should("be.visible")
    cy.get('form > table > tbody > :nth-child(1) > :nth-child(1)').should('contain', 'Participant Study Code:')
    cy.get(':nth-child(1) > .small').should('contain', '(Study ID)')
    cy.get('#participant_study_id').type('12345')

    cy.get('form > table > tbody > :nth-child(2) > :nth-child(1)').should('contain', 'Activities to continue:')
    cy.get('#activities').type('My activities that will continue')
    cy.get('form > table > tbody > :nth-child(3) > td').should('contain', 'Justification for continuation of activities listed above:')
    cy.get('#justification').type('My justification for continuation of activities listed above.')
    ...
    cy.get(':nth-child(1) > strong').should('contain', 'Participant Study Code (Study ID):')
    cy.get(':nth-child(1) > .textAnswer').should('contain', '12345')
    cy.get(':nth-child(2) > strong').should('contain', 'Activities to continue:')
    cy.get(':nth-child(2) > .textAnswer').should('contain', 'My activities that will continue')
    cy.get(':nth-child(3) > strong').should('contain', 'Justification for continuation of activities listed above:')
    cy.get(':nth-child(3) > .textAnswer').should('contain', 'My justification for continuation of activities listed above.')
 ...
 it('Protocol Continuation - Being Routed', () => {
    ...
    cy.get('input#username.long')
    .type('maddoxb').should('have.value', 'maddoxb')  
    cy.get('input#password.long')
    .type('test{Enter}')
...
it('Protocol Continuation - Verify Study History/Submission Details', () => {
  ...
  cy.get('td[data-title="Accepted By IRB"] > .bucket > :nth-child(2) > table > tbody > :nth-child(10) > .countTotal > a').click()
  cy.wait(1000)
  cy.get('#bucketDataTable_filter > label > input').type('A Comparison of Two Brief Suicide Prevention Interventions Tailored for Youth on the Autism Spectrum')
...
//Verify header info
  cy.get('#irbInfo > :nth-child(1) > :nth-child(1)').should('contain', '21-2110')
  ...
  cy.get('#irbInfo > :nth-child(2) > :nth-child(4)').should('contain', '(Continuation)')
  cy.get(':nth-child(1) > strong').should('contain', 'PI:')
  cy.get('#irbInfo > :nth-child(3) > :nth-child(1)').should('contain', 'PI: Maddox, Brenna')
  ...
  cy.get('#irbInfo > :nth-child(3) > :nth-child(3)').should('contain', 'Admin Dept: Psychiatry - Research')
  ...
  cy.get(':nth-child(2) > :nth-child(2) > strong').should('contain', 'Participant Study Code (Study ID):')
  cy.get(':nth-child(2) > .textAnswer').should('contain', '12345')
  cy.get(':nth-child(2) > :nth-child(3) > strong').should('contain', 'Activities to continue:')
  cy.get(':nth-child(3) > .textAnswer').should('contain', 'My activities that will continue')
  cy.get(':nth-child(4) > strong').should('contain', 'Justification for continuation of activities listed above:')
  cy.get(':nth-child(4) > .textAnswer').should('contain', 'My justification for continuation of activities listed above.')
...
it('Protocol Continuation - Waiting PI Response', () => {
  ...
  cy.get(':nth-child(2) > strong').should('contain', 'Participant Study Code (Study ID):')
  cy.get(':nth-child(2) > .textAnswer').should('contain', '12345')
  cy.get(':nth-child(3) > strong').should('contain', 'Activities to continue:')
  cy.get(':nth-child(3) > .textAnswer').should('contain', 'My activities that will continue')
  cy.get(':nth-child(4) > strong').should('contain', 'Justification for continuation of activities listed above:')
  cy.get(':nth-child(4) > .textAnswer').should('contain', 'My justification for continuation of activities listed above.')
  ...
  cy.get('.cke_wysiwyg_frame').then(function($iframe) {
    const $body = $iframe.contents().find("body");
    console.log($body);
    cy.wrap($body[0]).type("My test Protocol Continuation Response for question 1");
...
//  cy.get('#navResubmit > :nth-child(2)').click()
 ...
 cy.get('#irbInfo > :nth-child(1) > div').should('contain', 'PI: Maddox, Brenna')
 cy.get('#irbInfo > :nth-child(2) > :nth-child(1)').should('contain', '21-2110')
 ...
 cy.get('#irbInfo > :nth-child(3) > div').should('contain', '(Continuation)')
 ...
 cy.get('.responseDiv > :nth-child(2)').should('contain', 'My test Protocol Continuation Response for question 1')
 //Action
