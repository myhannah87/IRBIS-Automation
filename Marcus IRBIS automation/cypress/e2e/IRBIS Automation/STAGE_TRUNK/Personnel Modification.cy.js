describe('Personnel Modification',  () => {
   it('Create Personnel Modification', () => {
        cy.visit('https://orisstage.research.unc.edu/irb/index.cfm');  //IRB ID: 22-0254
...
it('Personnel Modification - Accept for Review', () => {
 ...
 it('Personnel Modification - Verify Application/Review popup', () => {
      ...
      //Verify left rail
        ...
        cy.get('#pendingCOIReview-Text').should('contain', 'Pending COI Review (when checked)') 
        ...
        cy.get('#modalPopup').should("be.visible")
        cy.get('#hasTemplates > [style="display: block;"]').should('contain', 'Minor Stips - Expedited')
        ...
        cy.get('.ui-dialog').should("be.visible")
        cy.get('.ui-dialog-content > div').should('contain', 'Are you sure you want to finalize this letter?') 
        cy.get('.ui-dialog-buttonset > .btn-default').should('contain', 'Cancel')
        cy.get('.ui-dialog-buttonset > .btn-info').should('contain', 'Finalize').click()
    ...
    it('Personnel Modification - Waiting PI Response', () => {
      ...
      cy.get('#personnelModificationStipulationText').should('contain', 'My test PM stip')
      cy.wait(1000)
  ...
  it.only('Personnel Modification - Verify Resubmitted Review popup', () => {
        cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=general.logout');
       // cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=admin.reviewWindow&reviewId=592703&appId=472723');
        cy.get('input#username.long')
        .type('cdcantre').should('have.value', 'cdcantre')  
        //.type('cjfennim').should('have.value', 'cjfennim') 
        cy.get('input#password.long')
        ...
        cy.get('#stopListCOI').should('have.class', 'stopIcon pass')
        cy.get('#stopListManagementPlan').should('have.class', 'stopIcon na')
        cy.get('#stopListHSP').should("be.visible").should('have.class', 'stopIcon attention')
        cy.get('#stopListGCP').should('have.class', 'stopIcon na')
        cy.get('#closeNavContainer').click() 
