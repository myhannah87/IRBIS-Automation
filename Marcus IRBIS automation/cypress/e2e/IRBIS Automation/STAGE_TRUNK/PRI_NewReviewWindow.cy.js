describe('Promptly Reportable Information (PRI)',  () => {
   it('Create PRI Application', () => {
        cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=home.dashboard.irbstudymanagement&irb_id=06-0006');
   ...
   it('PRI Being Routed', () => {
    ...
    it('PRI Verify Study History/Submission Details', () => {
     ...
     it('PRI Verify Application/Review popup', () => {
        cy.visit('https://orisstage.research.unc.edu/irb/index.cfm');
        cy.viewport(1200, 700)
        //cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=admin.reviewWindow&reviewId=592858');
        cy.get('input#username.long')
        .type('geers').should('have.value', 'geers')  
        //.type('cjfennim').should('have.value', 'cjfennim') 
        cy.get('input#password.long')
        ...
        cy.get('#irbInfo > :nth-child(1) > :nth-child(1)').should('contain', '06-0006')
        cy.get('#irbInfo > :nth-child(2) > :nth-child(1)').should('contain', 'Promptly Reportable Information')
        cy.get(':nth-child(1) > strong').should('contain', 'PI:')
        cy.get('#irbInfo > :nth-child(3) > :nth-child(1)').should('contain', 'Stephenson, John')
        ...
        cy.get('#irbInfo > :nth-child(3) > :nth-child(3)').should('contain', 'Office of the Vice Chancellor for Research')
       ...
       // cy.get('#navReviewConditions').should("be.visible")
        ...
        cy.get('#stopListChecklist').should("be.visible").should('have.class', 'stopIcon na')
        cy.get('#stopListPRIResponse').should("be.visible").should('have.class', 'stopIcon na')
        cy.get('#stopListModExpiration').should("be.visible").should('have.class', 'stopIcon na')
        cy.get('#stopListPairedPM').should("be.visible").should('have.class', 'stopIcon na')

        //Verify review conditions -> SUBMISSION
        cy.get('#optionsList > :nth-child(2)').should("be.visible")
        cy.get('#stopListFlags').should("be.visible").should('have.class', 'stopIcon pass')
        ...
        cy.get('#stopListHSP').should("be.visible").should('have.class', 'stopIcon na')
        ...
        cy.get('#eventHeaderInfo > tbody > :nth-child(1) > :nth-child(1)').should('contain', 'IRB Number:')
        cy.get('[width="40%"]').should('contain', '06-0006')
        cy.get('#eventHeaderInfo > tbody > :nth-child(2) > :nth-child(1)').should('contain', 'PI:')
        cy.get('#eventHeaderInfo > tbody > :nth-child(2) > :nth-child(2)').should('contain', 'Stephenson, John')
        cy.get('#eventHeaderInfo > tbody > :nth-child(3) > :nth-child(1)').should('contain', 'Sponsor')
        cy.get(':nth-child(3) > [colspan="3"]').should('contain', 'Cancer Research Institute')
        cy.get('#eventHeaderInfo > tbody > :nth-child(4) > :nth-child(1)').should('contain', 'Study Title')
        cy.get('#headerInfoTitle').should('contain', 'Shock Treatment')
        cy.get('#eventHeaderInfo > tbody > :nth-child(1) > :nth-child(3)').should('contain', 'Study Status:')
        cy.get('[width="60%"]').should('contain', 'Approved')
        cy.get('#eventHeaderInfo > tbody > :nth-child(2) > :nth-child(3)').should('contain', 'IRB:')
        cy.get('#eventHeaderInfo > tbody > :nth-child(2) > :nth-child(4)').should('contain', 'Biomedical')

        cy.get('#screen0').should('contain', 'Brief Description of Event')
        cy.get('#questionBriefDescription').should('contain', 'The brief description is limited to 200 characters.')
        cy.get('#questionId1 > .answerContainer > .paperPadding').should('contain', 'My Cypress PRI test')


        cy.get('#screen1').should('contain', 'Brochure (IB) Update')
        
        cy.get('#questionA00').should('contain', '1. Is this submission regarding an Investigator')
        cy.get('#questionId50 > .answerContainer > .paperPadding').should('contain', 'Yes')

        cy.get('#questionA01').should('contain', '2. Did the IB update change the risk assessment from what was previously approved?')
        cy.get('#questionId51 > .answerContainer > .paperPadding').should('contain', 'Yes')

        cy.get('#questionA02').should('contain', '3. Please explain why or why not this changes the risk assessment')
        cy.get('#questionId52 > .answerContainer > .paperPadding').should('contain', 'My PRI answer to Question 3')

        cy.get('#questionA03').should('contain', '4. Have any current or past subjects that are under the purview of UNC')
        cy.get('#questionId53 > .answerContainer > .paperPadding').should('contain', 'Yes')

        cy.get('#questionA04').should('contain', '5. Please provide a reconsent or verbal notification plan that includes current and past participation')
        cy.get('#questionId54 > .answerContainer > .paperPadding').should('contain', 'My PRI answer to Question 5')

        cy.get('#screen2').should('contain', 'Does This Promptly Reportable Information Need to be Reported?')
       
        cy.get('#questionA1').should('contain', 'A1. Did this event occur at a site for which a UNC-Chapel Hill IRB has direct oversight responsibility or involve a research participant at one of those sites?')
        cy.get('#questionId2 > .answerContainer > .paperPadding').should('contain', 'Yes')
        cy.get('#questionA1Sites').should('contain', 'Please indicate which site or sites where this PRI applies.')
        cy.get('#questionId68 > .answerContainer > .paperPadding').should('contain', 'None')

        cy.get('#questionA2Yes').should('contain', 'A2. Was the event unexpected in nature, severity, or frequency? (See SOP 1401)')
        cy.get('#questionId4 > .answerContainer > .paperPadding').should('contain', 'Yes')

        cy.get('.subQuestion').should('contain', 'Please explain all responses:')
        cy.get('.subQuestion > div').should('contain', 'My explanation for question A2')

        cy.get('#questionA3').should('contain', 'A3. Do you think the event was related or possibly related to this research?')
        cy.get('#questionId6 > .answerContainer > .paperPadding').should('contain', 'Yes')

        cy.get('#questionA4').should('contain', 'A4. Does the event suggest that the research places subjects or others at a greater risk of harm than was previously known or recognized?')
        cy.get('#questionId8 > .answerContainer > .paperPadding').should("be.visible")
        cy.get('.paperPadding > :nth-child(13)').should('contain', 'My explanation of my responses from A4')

        cy.get('#questionA5').should('contain', 'Does the event involve a potential(1) inappropriate sharing or disclosure of a participant')

        cy.get('#questionId55 > .answerContainer > .paperPadding').should('contain', 'No')

        cy.get('#questionA6').should('contain', 'A6. Has this event or issue been identified as noncompliance previously for either this study or other studies that the PI has oversight of')
        cy.get('#questionId56 > .answerContainer > .paperPadding').should('contain', 'Yes')

        cy.get('#questionA7').should('contain', 'A7. In review of the study’s deviation log, are there any similar issues or events?')
        cy.get('#questionId58 > .answerContainer > .paperPadding').should('contain', 'Yes')

        cy.get('#questionA8').should('contain', 'A8. Is this submission regarding other Promptly Reportable Information as outlined in SOP 1401?')
        ...
        cy.get('#questionId15 > .answerContainer > .paperPadding').should('contain', '01/01/2024')

        cy.get('#questionB2').should('contain', 'B2. Location of Event:')
        cy.get('#questionId16 > .answerContainer > .paperPadding').should('contain', 'Chapel Hill')

        cy.get('#questionB3').should('contain', 'B3. Full Description of Event.')
        cy.get('#questionId17 > .answerContainer > .paperPadding').should('contain', 'My Cypress PRI test description')

        cy.get('#screen4').should('contain', 'Was this a serious adverse event?')

        cy.get('#questionC1').should('contain', 'C1. Did the event result in death?')
        cy.get('#questionId67 > .answerContainer > .paperPadding').should('contain', 'No')

        cy.get('#questionC2').should('contain', 'C2. Was the event life-threatening (i.e., placed the subject at immediate risk of death from the event, as it occurred)?')
        cy.get('#questionId21 > .answerContainer > .paperPadding').should('contain', 'No')

        cy.get('#questionC3').should('contain', 'C3. Did the event result in inpatient hospitalization or prolongation of existing hospitalization?')
        cy.get('#questionId21 > .answerContainer > .paperPadding').should('contain', 'No')

        cy.get('#screen5').should('contain', 'Protocol/Consent Forms')
        cy.get('#questionD1').should('contain', 'D1. Given this event\'s occurrence, are there revisions to the study or consent documents that you would like to submit at this time?')
        cy.get('#questionId32 > .answerContainer > .paperPadding').should('contain', 'Describe your changes below: My description of my changes for question D1')

        cy.get('#screen6').should('contain', 'Root Cause Analysis and Corrective and Preventative Action (CAPA) Plan')
        cy.get('#questionE1').should('contain', 'Root Cause Analysis:')
        cy.get('#questionId35 > .answerContainer > .paperPadding').should('contain', 'My response for E1')

        cy.get('#questionE2').should('contain', 'Corrective and Preventative Actions:')
        cy.get('#questionId36 > .answerContainer > .paperPadding').should('contain', 'My response for E2')

        cy.get('#questionE3').should('contain', ' Plan for Effectiveness Check:')
        cy.get('#questionId37 > .answerContainer > .paperPadding').should('contain', 'My response for E3')
        ...
        cy.get('#screenEmailManagement').should('contain', 'Email Management for IRB NO: 06-0006')
        
        //CREATE STIP
        cy.get('#addStipForQuestion8').click()
        cy.get('#questionId8 > #stip0 > .stipHeader').should('contain', 'Stipulation: by Eric Geers on')
        ...
        cy.get('.editStipulation > p').should('contain', 'My test PRI stip for question A4')
        ...
        cy.get('#PIResponseDueDateContainer > :nth-child(1)').should('contain', 'PI Response Due Date') 
        cy.get('#PIResponseDueDateAlert').should('contain', 'PI Response Due Date is required') 
        cy.get('#PIResponseDueDateContainer > .reviewResultContainer > .ui-datepicker-trigger > img').click()
...
it.only('Resubmitted PRI - Verify Application/Review popup', () => {
        ...
        cy.get('#questionPIResponse8 > :nth-child(1)').should('contain', 'Stipulation/Response #1') 
        cy.get('.stipDiv > :nth-child(1)').should('contain', 'Stipulation by Eric Geers on') 
        cy.get('.stipDiv > :nth-child(2) > p').should('contain', 'My test PRI stip for question A4')
        
        cy.get('.responseDiv > :nth-child(1)').should('contain', 'Response by  Marcus Hannah on') 
        cy.get('.responseDiv > :nth-child(2) > p').should('contain', 'My test PRI Response for question A4')
        ...
        //RECORD RESULTS     
        cy.get('#reviewResult').should('contain', 'Closed')
        ...
        cy.get('#PRIDetermination > :nth-child(1)').should('contain', 'a. Noncompliance or No Determination')
        ...
        cy.get('#PRIDetermination > :nth-child(5)').should('contain', 'e. Complaint')
        cy.get('#PRIDetermination > :nth-child(6)').should('contain', 'f. Allegations of Noncompliance')
        cy.get('#PRIDetermination > :nth-child(7)').should('contain', 'g. Suspension')
        cy.get('#PRIDetermination > :nth-child(8)').should('contain', 'h. Termination')
        cy.get('#PRIDetermination > :nth-child(9)').should('contain', 'i. Regulatory Agency Audit (e.g., FDA, DEA, DoD)')
