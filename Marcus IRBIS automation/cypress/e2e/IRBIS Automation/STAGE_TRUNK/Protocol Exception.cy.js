describe('Protocol Exception',  () => {
 it('Create Protocol Exception', () => {
  ...
  //cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=home.dashboard.irbStudyManagement&irb_id=22-2824');
  ...
  cy.get('#irbInfo > .pageHeaderTitle').should('contain', 'UNC IRB Protocol Exception Request')
  ...
  cy.get('#formBody > .pageHeaderTitle').should('contain', 'Protocol Exception')
  ...
  //question 1
  cy.get('#questionList > :nth-child(1) > :nth-child(2) > div').should('contain', 'Participant Study Identification Code (Study ID)')
  cy.get('#participant_study_id').type('My Participant Study Identification Code')
 ...
 it('Protocol Exception - Being Routed/PI Cert', () => {
  cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=general.logout');
  //cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=home.protocolException&id=383967');
  cy.get('input#username.long')
  .type('rtraub').should('have.value', 'rtraub')  
  cy.get('input#password.long')
  .type('test{Enter}')
  ...
  cy.get('input[placeholder="Title"]')
      .type('Retrospective review of patients switched from plasma exchange therapy to efgartigimod treatment')
  cy.get('.sorting_1 > a').click()
  

  cy.get('#navSubmitToRouting > :nth-child(2) > .small').should('contain', 'Submitted:')
  cy.get('#navPICert > :nth-child(2)').click()
  //cy.wait(5000)
  cy.get('#navPICert > :nth-child(2) > .small').should('contain', 'Certified:')
...
it('Protocol Exception - Verify Study History/Submission Details', () => {
  ...
  cy.wait(1000)
  cy.get('#bucketDataTable_filter > label > input').type('Retrospective review of patients switched from plasma exchange therapy to efgartigimod treatment')
 ...
 it('Protocol Exception - Verify Application/Review popup', () => {
  ...
  //Verify header info
  cy.get('#irbInfo > :nth-child(1) > :nth-child(1)').should('contain', '22-2824')
  cy.get('#irbInfo > :nth-child(1) > :nth-child(3)').should('contain', 'Ref ID:')
  cy.get('#irbInfo > :nth-child(2) > :nth-child(1)').should('contain', 'Protocol Exception')
  cy.get('#irbInfo > :nth-child(2) > :nth-child(4)').should('contain', '(Protocol Exception)')
  cy.get(':nth-child(1) > strong').should('contain', 'PI:')
  cy.get('#irbInfo > :nth-child(3) > :nth-child(1)').should('contain', 'PI: Traub, Rebecca')
  ...
  cy.get('#irbInfo > :nth-child(3) > :nth-child(3)').should('contain', 'Admin Dept: Neurology-Neuromuscular')

  //Create a Review
  cy.get('#navNextStep').should('contain', 'Next Step: Add Review Type').click()
  cy.get('#modalPopup').should("be.visible")
  cy.get('#hasReviewTypes > [style="display: block;"]').should('contain', "Create review type : 'Administrative'?")
  ...
  //Question 1
  cy.get('#questionList > :nth-child(1) > :nth-child(2) > :nth-child(1)').should('contain', 'Participant Study Identification Code (Study ID) – Do not provide any direct subject identifiers. Instead, use a code identifier such as the one used in the study records.')
  cy.get(':nth-child(1) > :nth-child(2) > .textAnswer').should('contain', 'My Participant Study Identification Code') 
  cy.get(':nth-child(1) > .questionNumber > .addProtocolEventStipButton').should("be.visible")

  //Question 2
  cy.get('#questionList > :nth-child(2) > :nth-child(2) > :nth-child(1)').should('contain', 'Provide a description and rationale for the request.') 
  cy.get(':nth-child(2) > :nth-child(2) > .textAnswer').should('contain', 'My description and rationale for the request.') 
  cy.get(':nth-child(2) > .questionNumber > .addProtocolEventStipButton').should("be.visible")

  //Question 3
  cy.get('#questionList > :nth-child(3) > :nth-child(2)').should('contain', 'Does the protocol exception pose new risks not currently listed in the consent form, increase the likelihood or severity of experiencing known risks, or otherwise affect the safety of the participant?') 
  cy.get(':nth-child(3) > :nth-child(2) > :nth-child(1) > .glyphicon-check').should("be.visible")
  cy.get(':nth-child(3) > :nth-child(2) > :nth-child(1) > .glyphicon-unchecked').should("be.visible")
  cy.get(':nth-child(3) > :nth-child(2) > .textAnswer').should('contain', 'My justification regarding the response.') 
  cy.get(':nth-child(3) > .questionNumber > .addProtocolEventStipButton').should("be.visible")

  //Question 4
  cy.get('#questionList > :nth-child(4) > :nth-child(2)').should('contain', 'Does the protocol exception require a different informed consent form or process than the one currently approved by the IRB?') 
  cy.get(':nth-child(4) > :nth-child(2) > :nth-child(1) > .glyphicon-check').should("be.visible")
  cy.get(':nth-child(4) > :nth-child(2) > :nth-child(1) > .glyphicon-unchecked').should("be.visible")
  cy.get(':nth-child(4) > :nth-child(2) > :nth-child(3)').should('contain', 'Explain the consenting process you will use in relation to this protocol exception and if applicable, attach any proposed addendum to the informed consent that will be used.') 
  cy.get(':nth-child(4) > :nth-child(2) > :nth-child(3) > .textAnswer').should('contain', 'My explanation of the consenting process I will use in relation to this protocol exception and if applicable.') 
  cy.get(':nth-child(4) > .questionNumber > .addProtocolEventStipButton').should("be.visible")

  //Question 5
  cy.get('#questionList > :nth-child(5) > :nth-child(2)').should('contain', 'Does the protocol exception affect the integrity of the study data?') 
  cy.get(':nth-child(5) > :nth-child(2) > :nth-child(1) > .glyphicon-check').should("be.visible")
  cy.get(':nth-child(5) > :nth-child(2) > :nth-child(1) > .glyphicon-unchecked').should("be.visible")
  cy.get(':nth-child(5) > :nth-child(2) > .textAnswer').should('contain', 'My justification regarding your response.') 
  cy.get(':nth-child(5) > .questionNumber > .addProtocolEventStipButton').should("be.visible")

  //Question 6
  cy.get(':nth-child(6) > :nth-child(2)').should('contain', 'Will data collected as a result of the exception be analyzed in a different manner from other collected data?')
  cy.get(':nth-child(6) > :nth-child(2) > :nth-child(1) > .glyphicon-check').should("be.visible")
  cy.get(':nth-child(6) > :nth-child(2) > :nth-child(1) > .glyphicon-unchecked').should("be.visible")
  cy.get('[data-showid="data_analyzed_different_yes"]').should('contain', 'Explain how it will be analyzed differently.')
  cy.get('[data-showid="data_analyzed_different_yes"] > .textAnswer').should('contain', 'My explanation of how it will be analyzed differently.') 
  cy.get(':nth-child(6) > .questionNumber > .addProtocolEventStipButton').should("be.visible")

  //Question 7
  cy.get('#questionList > :nth-child(7) > :nth-child(2)').should('contain', 'Have you previously requested this exception for this same reason?')
  cy.get(':nth-child(7) > :nth-child(2) > :nth-child(1) > .glyphicon-check').should("be.visible")
  cy.get(':nth-child(7) > :nth-child(2) > :nth-child(1) > .glyphicon-unchecked').should("be.visible")
  cy.get('[data-showid="previously_requested_yes"]').should('contain', 'My explanation of if a previous exception was requested for the same reason.')
  cy.get(':nth-child(7) > .questionNumber > .addProtocolEventStipButton').should("be.visible")

  //Question 8
  cy.get(':nth-child(8) > :nth-child(2)').should('contain', 'Is this study externally sponsored?')
  cy.get(':nth-child(8) > :nth-child(2) > :nth-child(1) > .glyphicon-unchecked').should("be.visible")
  cy.get(':nth-child(8) > :nth-child(2) > :nth-child(1) > .glyphicon-check').should("be.visible")

  //Question 9
  cy.get('#questionList > :nth-child(9) > :nth-child(2)').should('contain', 'For participants not meeting inclusion/exclusion criteria, a physician uninvolved in the care of the participant must provide a written endorsement for the inclusion of the ineligible person due to clinical alternatives being limited to less favorable options.')
  cy.get(':nth-child(9) > :nth-child(2) > :nth-child(3) > .glyphicon-unchecked').should("be.visible")
  cy.get(':nth-child(9) > :nth-child(2) > :nth-child(3) > .glyphicon-check').should("be.visible")
  cy.get('[data-showid="endorsement_uploaded_NA"]').should('contain', 'Please provide an explanation.')
  cy.get('[data-showid="endorsement_uploaded_NA"] > .textAnswer').should('contain', 'My provided explanation as to why a written endorsement been uploaded as an attachment')

  //Create a stipulation
  cy.get(':nth-child(1) > .questionNumber > .addProtocolEventStipButton').click()
  cy.wait(1000)
  ...
  cy.get('[style="display: flex;"] > :nth-child(2) > :nth-child(1)').should('contain', 'Participant Study Identification Code')
  cy.get('[style="display: flex;"] > :nth-child(2) > .textAnswer').should('contain', 'Participant Study Identification Code')
  cy.contains('Stipulation: by Celeste Cantrell on').should("be.visible")
  cy.get('.editStipulation > p').should('contain', 'My test stip for question 1')
  ...
  cy.get('.ui-dialog').should("be.visible")
  cy.get('#hasTemplates > [style="display: block;"]').should('contain', 'Protocol Exception - Minor Stips')
  cy.get(':nth-child(3) > [onclick="modalClose();"]').should('contain', 'Close')
  ...
  cy.get('.letterSubHeader').should('contain', 'Protocol Exception - Minor Stips')
  ...
  cy.get('#btnFinalizeLetterModal').should('contain', 'Finalize Letter').click()
...
it('Protocol Exceptions - Waiting PI Response', () => {
  ...
  //cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=home.protocolException&id=383860');
  cy.get('input#username.long')
          .type('mchopra').should('have.value', 'mchopra')  
  cy.get('input#password.long')
          .type('test{Enter}') 
  ...
  cy.get('#navStipulations > :nth-child(2)').should('contain', 'Stipulations')
  cy.get('#navSubmitToRouting > :nth-child(2)').should('contain', 'Submit Exception')
  cy.get('#navPICert > :nth-child(2)').should('contain', 'PI Certification')
  cy.get('#navDownloadPDF > :nth-child(2)').should('contain', 'Download PDF')
  //View history modal
  ...
  cy.get('#questionIdparticipant_study_id > :nth-child(2) > :nth-child(1)').should('contain', 'Participant Study Identification Code (Study ID)')
  cy.get('#questionIdparticipant_study_id > :nth-child(2) > .textAnswer').should('contain', 'My Participant Study Identification Code')
  ...
  const $body = $iframe.contents().find("body");
  console.log($body);
  cy.wrap($body[0]).type("My test Protocol Exception Response for question 1");
  ...
  cy.get('#navStipulations > :nth-child(2) > .small').should('contain', 'Responded: 1/1')

  cy.get('#navResubmit > :nth-child(2)').should('contain', 'Resubmit').click()
...
it.only('Protocol Exceptions - Verify Resubmitted Review popup', () => {
  cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=general.logout');
  //cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=admin.reviewWindow.PIResponses&masterId=383860&previousReviewId=285764&reviewId=531421&appId=416692');
  //cy.visit('https://orisstage.research.unc.edu/irb/index.cfm?event=admin.reviewWindow&reviewId=531421&appId=416692');
  ...
  cy.get('#irbInfo > :nth-child(1) > div').should('contain', 'PI: Traub, Rebecca')
  cy.get('#irbInfo > :nth-child(2) > :nth-child(1)').should('contain', '22-2824')
  cy.get('#irbInfo > :nth-child(2) > :nth-child(3)').should('contain', 'Ref ID:')
  cy.get('#irbInfo > :nth-child(3) > span').should('contain', 'Protocol Exception')
  cy.get('#irbInfo > :nth-child(3) > div').should('contain', '(Protocol Exception)')
  ...
  cy.get('.answerForStip > :nth-child(2) > :nth-child(2) > :nth-child(1)').should('contain', 'Participant Study Identification Code (Study ID) – Do not provide any direct subject identifiers. Instead, use a code identifier such as the one used in the study records.')
  cy.get('.answerForStip > :nth-child(2) > :nth-child(2) > .textAnswer').should('contain', 'My Participant Study Identification Code')
  //STIPLUATION/RESPONSE #1
  cy.get('#questionPIResponseparticipant_study_id > :nth-child(1)').should('contain', 'Stipulation/Response #1')
  cy.get('.stipDiv > :nth-child(1)').should('contain', 'Stipulation by Celeste Cantrell on')
  cy.get('.stipDiv > :nth-child(2) > p').should('contain', 'My test stip for question 1')
  //Response
  cy.get('.responseDiv > :nth-child(1)').should("be.visible")
  cy.get('.responseDiv > :nth-child(2)').should('contain', 'My test Protocol Exception Response for question 1')
  //Action
  cy.get('.actionDiv > :nth-child(1)').should('contain', 'Click the Desired Action')
  cy.get('.actionDiv > :nth-child(2)').should("be.visible")
  cy.get('.resend').click()
  cy.wait(5000)
  cy.get('.resendDiv > :nth-child(1)').should('contain', 'Resend (Create new stipulation)')
 ...
 //NAVIGATE BACK TO REVIEW WINDOW
  ...
  cy.get('#sent_to_email2').should("be.visible")
  cy.get('.cke_wysiwyg_frame').should("be.visible")
  cy.get('.letterSubHeader').should('contain', 'Protocol Exception - Acknowledged')
  cy.get('#btnViewPDF').should('contain', 'View PDF')
  cy.get('#btnDelete').should('contain', 'Delete')
  cy.get('#btnSaveLetter').should('contain', 'Save')
  cy.get('#btnSendToChair').should('contain', 'Send to Chair')
