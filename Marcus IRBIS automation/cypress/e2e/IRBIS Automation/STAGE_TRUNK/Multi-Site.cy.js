...
//2. PROJECT PERSONNEL SCREEN
   ...
   cy.get('[qid="q_2142"] > .copy > #q_2_2142').type('The Ohio State', { force: true })
   cy.get('[qid="q_15"] > td.copy > [inputvalue="External Site PI"] > #q_2_15').click()
...
//B.2 PROTECTED HEALTH INFORMATION (PHI)
        ...
        cy.get('#q_79_957').type('My Multi-Site IRB B.2.1 test - List the data you are planning to collect for this purpose.')
        cy.get('#tbl_79_956 > tbody > :nth-child(2) > td.copy').should("be.visible")
        cy.get('#q_79_958').type('My Multi-Site B.2.1 test - Describe how confidentiality/privacy will be protected prior to ascertaining the patients willingness to participate')
        cy.get('#tbl_79_956 > tbody > :nth-child(3) > td.copy').should("be.visible")
        cy.get('#q_79_959').type('My Multi-Site B.2.1 test - Describe when and how you will destroy PHI for individuals')
        cy.get(':nth-child(4) > td.copy').should("be.visible")
        cy.get('#q_79_2293').type('My Multi-Site B.2.1 test - Please explain how screening could not be practicably conducted without the waiver.')
        cy.get(':nth-child(5) > td.copy').should("be.visible")
        cy.get('#q_79_2294').type('My Multi-Site B.2.1 test - Explain why it would not be possible to conduct the screening without the use of PHI')
