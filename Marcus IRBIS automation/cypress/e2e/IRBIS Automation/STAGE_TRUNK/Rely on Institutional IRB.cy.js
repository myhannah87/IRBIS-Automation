...
//B.3 SUBJECT CONTACT, DURATION AND PRIVACY
        ...
        cy.get('#hd_2010\\|\\|6\\,3\\,10\\,12\\,16\\,4\\,11\\,15\\,2\\,14\\,13\\,17 > [width="99%"]').should("be.visible")
        cy.get('#tr_2009\\|\\|2 > td.copy > [style="position: relative; display: block;"] > :nth-child(3) > .copy').click()
        cy.get('[name="submit1"]').click()
...
//C.1 DATA SOURCES
  ...
  it('Rely on Institutional IRB Being Routed/PI Cert', () => {  
        ...
        cy.get('input#password.long')
         ...
         cy.get('input[placeholder="Title"]')
             .type('My Rely on Institutional IRB test')
 ...
 it('Rely on Institutional IRB - Local/PRC Dept Approval', () => {  
         ...
         cy.get('#dashboardmenu8 > dd.even > a').click()
         cy.get('input[placeholder="Title"]')
                 .type('My Rely on Institutional IRB Cypress Test')
         cy.get('.sorting_1 > a').click()
         cy.get('[style="padding:10px;"] > div.searchresults').should('contain', 'This submission is being routed to you because approval of your Department or Review Committee is required') 
         cy.get('#btnPDF').should("be.visible") 
         cy.get(':nth-child(2) > .button').should("be.visible") 
 ...
 it('Accept Rely on Institutional IRB For Review', () => {  
         ...
         cy.wait(1000)
         cy.get('#bucketDataTable_filter > label > input').type('My Rely on Institutional IRB Cypress Test')
 ...
 it('Verify Study History Screen', () => {  
        ...
        //STUDY DASHBOARD
         ...
         cy.get('[rowspan="2"]').should('contain', 'My Rely on Institutional IRB Cypress Test')
