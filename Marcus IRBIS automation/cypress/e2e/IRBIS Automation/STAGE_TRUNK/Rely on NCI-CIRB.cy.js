describe('Create Rely on NCI-CIRB Application',  () => {
  ...
  it('Create Rely on Commerical IRB', () => {
         ...
         cy.get('#wizard10 > .wizardButtons > .btn-primary').click()
        //cy.wait(15000)


//RELY ON INFORMATION
        cy.get('[qid="h_1984"] > [width="99%"]').should("be.visible")
        cy.get('[qid="q_1984"] > .copy').should("be.visible")
 ...
 //1. GENERAL INFORMATION SCREEN
         ...
         .type('My Rely on NCI-CIRB Cypress Test')//.should('have.value', 'My NHSR Regression Test') 
         cy.get('.cke_wysiwyg_frame').then(function($iframe) {
            const $body = $iframe.contents().find("body");
            console.log($body);
            cy.wrap($body[0]).type("My RELY ON NCI-CIRB CYPRESS TEST"); 
         cy.get('[name="submit1"]').click()
 ...
 //4.SCREENING QUESTIONS
        ...
        cy.get('[qid="q_443"] > [style="padding:5px 10px 5px 0px;"] > table > tbody > tr > .copy').should("be.visible")
        cy.get('[qid="q_34"] > [style="padding:5px 10px 5px 0px;"] > table > tbody > tr > .label').should("be.visible")
        cy.get('[qid="q_34"] > [style="padding:5px 10px 5px 0px;"] > table > tbody > tr > .copy').should("be.visible")
        cy.get('[qid="q_35"] > [style="padding:5px 10px 5px 0px;"] > table > tbody > tr > .label').should("be.visible")
        cy.get('[qid="q_35"] > [style="padding:5px 10px 5px 0px;"] > table > tbody > tr > .copy').should("be.visible")
...
//A.4.A BIOMEDICAL METHODS AND PROCEDURES
      ...
      //A.4.A.1 - Add investigational devices, drugs, biologics, or other substances pop up 
        ...
        cy.get('[qid="q_2230"] > .copy > #q_74_2230').type('My Rely On NCI-CIRB A.4.A.1 test')
        ...
        cy.get('[qid="q_2232"] > .copy > #q_74_2232').type('My Rely On NCI-CIRB IND/IDE test')
        cy.get('[qid="q_2233"] > td.copy > #q_74_2233').type('My Rely On NCI-CIRB Brief Description test')
...
//A.10 CONFIDENTIALITY OF THE DATA
        ...
        cy.get('#cke_1_contents > .cke_wysiwyg_frame').then(function($iframe) {
                const $body = $iframe.contents().find("body");
                console.log($body);
                cy.wrap($body[0]).type("My Rely On NCI-CIRB A.10.1 test");
        ...
        cy.get('#cke_2_contents > .cke_wysiwyg_frame').then(function($iframe) {
                const $body = $iframe.contents().find("body");
                console.log($body);
                cy.wrap($body[0]).type("My Rely On NCI-CIRB A.10.3 test");
        cy.get('[name="submit1"]').click()
...
//B.1 METHODS OF RECRUITING
        ...
        cy.get('.even > td.copy > [style="position: relative; display: block;"] > div > .copy').click()
        ...
        cy.get('#q_78_2292').type('My Rely On NCI-CIRB B.1.2 test')
...
//B.2 PROTECTED HEALTH INFORMATION (PHI)
        ...
        cy.get('#q_79_957').type('My Rely On NCI-CIRB B.2.1 test - List the data you are planning to collect for this purpose.')
        cy.get('#tbl_79_956 > tbody > :nth-child(2) > td.copy').should("be.visible")
        cy.get('#q_79_958').type('My Rely On NCI-CIRB B.2.1 test - Describe how confidentiality/privacy will be protected prior to ascertaining the patients willingness to participate')
        cy.get('#tbl_79_956 > tbody > :nth-child(3) > td.copy').should("be.visible")
        cy.get('#q_79_959').type('My Rely On NCI-CIRB B.2.1 test - Describe when and how you will destroy PHI for individuals')
        cy.get(':nth-child(4) > td.copy').should("be.visible")
        cy.get('#q_79_2293').type('My Rely On NCI-CIRB B.2.1 test - Please explain how screening could not be practicably conducted without the waiver.')
        cy.get(':nth-child(5) > td.copy').should("be.visible")
        cy.get('#q_79_2294').type('My Rely On NCI-CIRB B.2.1 test - Explain why it would not be possible to conduct the screening without the use of PHI')
