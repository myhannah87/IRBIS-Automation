...
//A.4.A BIOMEDICAL METHODS AND PROCEDURES
      ...
      //A.4.A.1 - Add investigational devices, drugs, biologics, or other substances pop up 
       // cy.get('#noansweravailable').should("be.visible")
        ...
        cy.get('[qid="q_2230"] > .copy > #q_74_2230').type('My Rely On Commerical IRB A.4.A.1 test')
        ...
        cy.get('[qid="q_2232"] > .copy > #q_74_2232').type('My Rely On Commerical IRB IND/IDE test')
        cy.get('[qid="q_2233"] > td.copy > #q_74_2233').type('My Rely On Commerical IRB Brief Description test')
...
//B.2 PROTECTED HEALTH INFORMATION (PHI)
       ...
       // cy.get('#tbl_79_956 > tbody > :nth-child(1) > td.copy').should("be.visible")
        cy.get('#q_79_957').type('My Rely On Commerical IRB B.2.1 test - List the data you are planning to collect for this purpose.')
        cy.get('#tbl_79_956 > tbody > :nth-child(2) > td.copy').should("be.visible")
        cy.get('#q_79_958').type('My Rely On Commerical B.2.1 test - Describe how confidentiality/privacy will be protected prior to ascertaining the patients willingness to participate')
        cy.get('#tbl_79_956 > tbody > :nth-child(3) > td.copy').should("be.visible")
        cy.get('#q_79_959').type('My Rely On Commerical B.2.1 test - Describe when and how you will destroy PHI for individuals')
        cy.get(':nth-child(4) > td.copy').should("be.visible")
        cy.get('#q_79_2293').type('My Rely On Commerical B.2.1 test - Please explain how screening could not be practicably conducted without the waiver.')
        cy.get(':nth-child(5) > td.copy').should("be.visible")
        cy.get('#q_79_2294').type('My Rely On Commerical B.2.1 test - Explain why it would not be possible to conduct the screening without the use of PHI')
...
it('Rely on Commerical Being Routed/PI Cert', () => {  
       ...
       cy.get('input#password.long')
        ...
        cy.get('input[placeholder="Title"]')
            .type('My Rely on Commerical test')
...
it('Rely on Commerical - Local/PRC Dept Approval', () => {  
        ...
        cy.get('#dashboardmenu8 > dd.even > a').click()
        cy.get('input[placeholder="Title"]')
                .type('My Rely on Commerical IRB Cypress Test')
        cy.get('.sorting_1 > a').click()
        cy.get('[style="padding:10px;"] > div.searchresults').should('contain', 'This submission is being routed to you because approval of your Department or Review Committee is required') 
        cy.get('#btnPDF').should("be.visible") 
        cy.get(':nth-child(2) > .button').should("be.visible") 
...
it('Accept Rely on Commerical For Review', () => {  
        ...
        cy.wait(1000)
        cy.get('#bucketDataTable_filter > label > input').type('My Rely on Commerical IRB Cypress Test')
...
it('Verify Study History Screen', () => {  
       ...
       //STUDY DASHBOARD
        ...
        cy.get('[rowspan="2"]').should('contain', 'My Rely on Commerical IRB Cypress Test')
