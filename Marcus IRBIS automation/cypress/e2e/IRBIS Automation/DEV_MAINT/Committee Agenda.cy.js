...
it.only('Verify Committee Agenda', () => {
  cy.viewport(1400, 750) 
  cy.visit('https://orisdev.research.unc.edu/irb_maint/');
    //cy.get('title')
      //  .invoke('text')
        //.should('equal', '\nIRB \n') 
  ...
  cy.get('#documentList > :nth-child(1) > a').should('contain', 'View Agenda')
  cy.get('#documentList > :nth-child(2) > a').should('contain', 'View Minutes')
  ...
  // CAPTURE URL OF COMMITTEE AGENDA SCREEN
   ...
   cy.get('[aria-label="Result"] > .DataTables_sort_wrapper').should('contain', 'Result')
   cy.get('[aria-label="IRB Number: activate to sort column ascending"] > .DataTables_sort_wrapper').should('contain', 'IRB Number')
   cy.get('[aria-label="Title: activate to sort column ascending"] > .DataTables_sort_wrapper').should('contain', 'Title')
   cy.get('[aria-label="PI: activate to sort column ascending"] > .DataTables_sort_wrapper').should('contain', 'PI')
   cy.get('[aria-label="Type: activate to sort column ascending"] > .DataTables_sort_wrapper').should('contain', 'Type')
   cy.get('[aria-label="Date Routing Complete"] > .DataTables_sort_wrapper').should('contain', 'Date Routing Complete')
   cy.get('[aria-label="Approval Date"] > .DataTables_sort_wrapper').should('contain', 'Approval Date')
   cy.get('[aria-label="Expiration Date"] > .DataTables_sort_wrapper').should('contain', 'Expiration Date')
   cy.get('[aria-label="Review Type: activate to sort column ascending"] > .DataTables_sort_wrapper').should('contain', 'Review Type')
   cy.get('[aria-label="Reviewer: activate to sort column ascending"] > .DataTables_sort_wrapper').should('contain', 'Reviewer')
   cy.get('[aria-label="Secondary Reviewer: activate to sort column ascending"] > .DataTables_sort_wrapper').should('contain', 'Secondary Reviewer')
   cy.get('[aria-label="Review Result: activate to sort column ascending"] > .DataTables_sort_wrapper').should('contain', 'Review Result')
   cy.get('[aria-label="Sponsor: activate to sort column ascending"] > .DataTables_sort_wrapper').should('contain', 'Sponsor')
   ...
   cy.get('.trimHeight').should('contain', 'Dynamic neural mechanisms of brexanolone-induced antidepressant effects in postpartum depression: A feasibility study')
   ...
   cy.get('.odd > :nth-child(14)').should('contain', 'Baszucki Brain Research Fund')
  ...
  cy.get('#documentList > :nth-child(1) > a')
    ...
    .then((href) => {
  cy.visit('orisdev.research.unc.edu/irb_maint/' + href)
  
  //Verify Agenda
  cy.get(':nth-child(1) > :nth-child(1) > h2').should('contain', 'Agenda')
  cy.get('#documentContainer > :nth-child(1) > :nth-child(1)').should("be.visible")
  cy.get('[data-outer="ol"][data-listtypeclass=""] > :nth-child(1)').should('contain', 'FULL BOARD REVIEWS')
  cy.get('[data-hid="445058B2433575FE13D840960643BBAC"] > :nth-child(1)').should('contain', 'Promptly Reportable Information')
  cy.get('[data-hid="445058B2433575FE13D840960643BBAC"] > :nth-child(3)').should('contain', '20-1632')
  cy.get('[data-hid="BA8537478F92850B00BB83D3EFA78504"] > .containerBlock > :nth-child(1) > b').should('contain', 'Primary Reviewer')
  cy.get('[data-hid="445058B2433575FE13D840960643BBAC"] > .containerBlock > :nth-child(2) > b').should('contain', 'Secondary Reviewer')
  cy.get('[data-hid="445058B2433575FE13D840960643BBAC"] > .containerBlock > .btnNewBlock').should('contain', 'Create Additional Block')
  cy.get('.titleBar').should("be.visible")
  cy.get('#lockedContainer').should('contain', 'Agenda is currently locked')
  cy.get('#reason').should("be.visible")
  cy.get('#unlockDocument').should('contain', 'Unlock')
  cy.get('[style="display:inline-block;"] > strong').should('contain', 'Export Current:')
 ...
 //Verify Minutes
  ...
  cy.get('#sideBar > :nth-child(9)').should('contain', 'Minutes Locked By:')
  cy.get('#sideBar > :nth-child(13)').should('contain', 'Minutes Publish Agenda Date:')
  cy.get('#sideBar > :nth-child(16)').should('contain', 'History (Readonly):')
  cy.get('[style="display:inline-block;"] > strong').should('contain', 'Export Current:')
...
it('Committee Availability', () => {
  ...
  cy.get('#leavingEarly')    
      .select('4:30 PM')
  cy.get('[aria-labelledby="ui-dialog-title-unavailableModal"] > .ui-dialog-buttonpane > .ui-dialog-buttonset > :nth-child(1)').click()
