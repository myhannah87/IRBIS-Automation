describe('Administrative Review',  () => {
   it('Create Administrative Review Application', () => {
        cy.visit('https://orisdev.research.unc.edu/irb_maint/index.cfm?event=home.dashboard.irbstudymanagement&irb_id=11-0190');
        ...
        cy.get('#alertBoxes > .pad10').should('contain', 'Your study is indicated for Annual Administrative Review')
        cy.get('#btnCreateRenewal').click()  
        
          //Administrative Review header area
        cy.get('#divNSIHeader > .hd').should('contain', 'Administrative Annual Review')
        cy.get(':nth-child(2) > tbody > :nth-child(1) > [width="14%"]').should('contain', 'IRB Number:')
        cy.get('.classToLoad').should('contain', '11-0190')

        cy.get(':nth-child(2) > tbody > :nth-child(2) > :nth-child(1)').should('contain', 'PI:')
        cy.get('tbody > :nth-child(2) > :nth-child(2) > a').should('contain', 'Slattery, John')
        ...
        cy.get(':nth-child(2) > tbody > :nth-child(1) > :nth-child(3)').should('contain', 'Study Status:')
        cy.get(':nth-child(2) > tbody > :nth-child(1) > :nth-child(4)').should('contain', 'Approved')

        cy.get(':nth-child(2) > tbody > :nth-child(2) > :nth-child(3)').should('contain', 'IRB:')
        cy.get(':nth-child(2) > tbody > :nth-child(2) > :nth-child(4)').should('contain', 'Non-Biomedical')

        cy.get(':nth-child(2) > tbody > :nth-child(1) > :nth-child(5)').should('contain', 'Admin Annual Review Date:')
        cy.get(':nth-child(2) > tbody > :nth-child(1) > :nth-child(6)').should("be.visible")

        cy.get('[style="margin-top: 20px;"] > tbody > :nth-child(1) > [width="14%"]').should('contain', 'Reference Id:')
        cy.get(':nth-child(2) > [width="14%"]').should('contain', 'Date Submitted:').should('contain', 'Date Submitted:')

        cy.get('[style="margin-top: 20px;"] > tbody > :nth-child(1) > :nth-child(3)').should('contain', 'Submission Status:')
        ...
        cy.get('#divItemList > :nth-child(8)').should('contain', 'Submission Management')
       ...
       // cy.get('#itemListRevisions > dd').should('contain', 'View Revisions')
        ...
        cy.get('#divEFORMScreen > :nth-child(3)').should('contain', 'Administrative Annual Review Submission')
        cy.get('#divAdminReviewForm1 > .hd').should('contain', 'Current Study Status')

        cy.get('.bold > .Q1').should('contain', '1. Choose only one:')
        cy.get(':nth-child(2) > .Q1').should('contain', 'A) Data collection continues')
        cy.get(':nth-child(3) > .Q1').should('contain', 'B) All data has been collected/obtained')
      
        //Verify triggered options under option B
        cy.get('#renewalActionRequestedB').click()
        cy.get('#closeMyStudy_sub > .questionBlock').should('contain', 'NOTE, the study may be closed if:')
        cy.get('#closeMyStudy_sub > .questionBlock > ul > :nth-child(1)').should('contain', 'All identifying information has been destroyed')
        cy.get('#closeMyStudy_sub > .questionBlock > ul > :nth-child(2)').should('contain', 'Analysis of identifiable data is complete and you were approved to store identifiers beyond closure of your study')
        cy.get('.closeMyStudyAnchor').should('contain', 'Click here to close the study')
        ...
        cy.get('#divAdminReviewForm2 > .bd > .questionHouse > .bold').should('contain', '2. Number of Subjects involved through direct contact or use of their data')
        cy.get('#divAdminReviewForm2 > .bd > .questionHouse > :nth-child(2)').should('contain', 'Total number as currently approved by IRB')
        ...
        cy.get('.\\32 C').should('contain', 'C. Number of subjects included/enrolled since last renewal')
        cy.get('#NumberSubjectsIncludedLastRenewal').type('0')

        cy.get('.\\32 D').should('contain', 'D. Number to be included/enrolled in upcoming year')
        cy.get('#NumberSubjectsIncludedUpcomingYear').type('0')

        cy.get('span.\\32 E').should('contain', 'Since initial approval, have any subjects been enrolled who are members of vulnerable/protected groups')
        cy.get('#protectedGroups2').click()
        cy.get('#protectedGroups_sub').should("not.be.visible")
                
        cy.get('#protectedGroups1').click()
        cy.get('#protectedGroups_sub > :nth-child(1)').should('contain', 'Please enter the number of subjects enrolled for each group:')
        cy.get('#protectedGroups_sub > :nth-child(2)').should('contain', 'Children:')
        cy.get('#Children').type("1")

        cy.get('#protectedGroups_sub > :nth-child(3)').should('contain', 'Pregnant women:')
        cy.get('#PregnantWomen').type("2")

        cy.get('#protectedGroups_sub > :nth-child(4)').should('contain', 'Nonviable neonates or neonates of uncertain viability')
        cy.get('#Neonates').type("3")

        cy.get('#protectedGroups_sub > :nth-child(5)').should('contain', 'Prisoners')
        cy.get('#Prisoners').type("4")

        cy.get('#protectedGroups_sub > :nth-child(6)').should('contain', 'Decisionally impaired individuals')
        ...
        cy.get('#shortForm_sub > :nth-child(1)').should('contain', 'How many subjects?')
        cy.get('#NumberSubjects').type("1")
        cy.get('#shortForm_sub > :nth-child(2)').should('contain', 'Explain why you were not able to provide a full, translated consent form:')
        cy.get('#Explain_ShortForm').type("My explanation of why I wasn't able to provide a full, translated consent form.")
        ...
        cy.get('.bd > .questionBlock > :nth-child(3)').should('contain', 'If there are any events that meet the reporting requirements of New Safety Information (NSI) as defined in OHRE SOP 1401 that has not yet been reported, please submit an NSI at this time.')
        cy.get('.questionBlock > :nth-child(4)').should('contain', 'All other deviations should be documented in your study records in a Protocol Deviation Tracking Log.')
       ...
       it('Administrative Review - Being Routed', () => {
        ...
        cy.get('input[placeholder="Title"]')
            .type('Shock Treatment 2.0')
        cy.get('.sorting_1 > a').click()

        cy.get('#itemListPICert > dd').click()
        cy.get('[style="padding: 10px; text-align: center;"] > .searchresults').should('contain','Please review the following Administrative Annual Review Form. Certify below to finalize submission to the IRB.')
      ...
      it('Administrative Review - Accept for Review', () => {
        ...
        cy.get('td[data-title="Submitted To IRB"] > .bucket > :nth-child(2) > table > tbody > :nth-child(8) > .countTotal > a').click()
        cy.wait(1000)
        cy.get('#bucketDataTable_filter > label > input').type('Shock Treatment 2.0')
       // cy.pause()
      ...
      //  cy.wait(3000) 
       ...
       it('Administrative Review - Verify Study History/Submission Details', () => {
        ...
        cy.get('td[data-title="Accepted By IRB"] > .bucket > :nth-child(2) > table > tbody > :nth-child(7) > .countTotal > a').click()
        cy.viewport(1200, 700)   
        cy.wait(1000)
        cy.get('#bucketDataTable_filter > label > input').type('Shock Treatment 2.0')
        cy.get('#bucketDataTable > tbody > .odd > :nth-child(1) > a').click()
     ...
     it('Administrative Review - Verify Application/Review popup', () => {
        ...
        cy.wait(2000)
        cy.reload()
        cy.get('.stipContainer').should('contain', 'My test AR stip for question 2B')
  ...
  it('Administrative Review - Waiting PI Response', () => {
        ...
        cy.get('#modalPopup').should("be.visible")
        cy.get('.alertMessageBox').should('contain', 'Are you ready to resubmit this Administrative Annual Review Form?')
        cy.get('form > .copy').should('contain', 'Resubmit Administrative Review').click()
  ...
  it('Administrative Review - Verify Resubmitted Review popup', () => {
        ...
        cy.get('td[data-title="PI Responses"] > .bucket > :nth-child(2) > table > tbody > :nth-child(8) > .countTotal > a').click()
        cy.viewport(1200, 700)   
        cy.wait(1000)
        cy.get('#bucketDataTable_filter > label > input').type('Shock Treatment 2.0')
        cy.get('#bucketDataTable > tbody > .odd > :nth-child(1) > a').click()
        ...
        cy.get('[style="float:right;"] > strong').should('contain', 'IRB No')
        cy.get('[style="float:right;"] > .link').should('contain', '11-0190')
        cy.get('[style="float:right; text-align:left;"]').should('contain', 'Administrative Review')
        ...
        cy.get("@recordResult").should("be.called")
         ...
         cy.get('[width="100%"] > .searchresults').should('contain', 'This letter is in draft and not finalized.')
        ...
        //FINALIZE ADMINISTRATIVE REVIEW - ACKNOWLEDGEMENT
         ...
         cy.get('#COIDisclosureStopAjaxLetter > span').should('contain', 'The letter cannot be finalized at this time.')
         cy.get('.air').should('contain', 'AIR COI Disclosures are not satisfied for this submission for each project personnel.')
         cy.get('.air > a').click()
         cy.get('.ui-dialog').should('be.visible')
         cy.get('.tableheaderirb > td').should('contain', 'Please provide a reason for the COI Disclosure Override and click')
